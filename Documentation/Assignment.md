...
<mark>I called the CMSIS function osSemaphoreAquire to wait until the task recieves the semaphore.</mark>
<br>
<br><br>
...
<mark>No, they take turns toggling because they share the same semaphore<br><br>

4.)	Now change one of the priorities of these two tasks, re-compile,  and re-run.
How has the behavior changed?
<mark>I made LED_D3 a higher priority and now its the only one that toggles because it gets priority with the semaphore<br><br>
...
><mark>the display toggles really fast between being dashes, incrementing, and decrementing<br><br><p>


>8.)	Is there a ‘priority’ associated with the Mutex?  If so, how can it be changed?
><br>  
><mark> Yes, the priority is low. That can be adjusted manually in the code<br><br>
<p>
...
><mark>To the eye it did not look like it affected it at all. Conceptually it would make it the lowest priority so it would only run when no other tasks are available to run<br><br>
>
---
<!--------------------------------------------------------------------------------->
...
><mark>You can have a one-shot timer that only runs one time<br><br>
...
>
><mark>it would allow for more scalabiltiy. It also removes the need for an additional task if you handle the debouncing in the timer task<br><br>
...
><mark>I created a new timer that handles the 7-segment refresh rate. In the callback of that timer I call the service routine for the 7-segment display. The slowest I could make the timer to make the persistance good was a 5 tick period<br><br>
